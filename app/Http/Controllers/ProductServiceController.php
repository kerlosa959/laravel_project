...
class ProductServiceController extends Controller
{
    public function index(Request $request)
    {
        ...
        if(\Auth::user()->can('manage product & service'))
        {
            ...
            if(!empty($request->category))
            {
                ...
                $productServices = ProductService::where('category_id', $request->category)->with(['category','unit'])->get();
            }
            else
            {
                $productServices = ProductService::with(['category','unit'])->get();
            }
    ...
    public function store(Request $request)
    {
        ...
        if(\Auth::user()->can('create product & service'))
        {
            ...
            {
                $productService->min_quantity        = $request->min_quantity;
            }
            else{
                $productService->min_quantity   = 0;
            }
    ...
    public function edit($id)
    {
        ...
        {
            if($productService->created_by == \Auth::user()->creatorId() || true)
            {
                $category = ProductServiceCategory::where('type', '=', 'product & service')->get()->pluck('name', 'id');
                $unit     = ProductServiceUnit::get()->pluck('name', 'id');
                $tax      = Tax::get()->pluck('name', 'id');
