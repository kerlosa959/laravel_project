...
//        dd($msg);
    ...
    public static function updateProductStock($product_id, $quantity)
    {
...
//                    dd(\Storage::disk(),$path);
    ...
    {
        $project_id='ALDAAR00001';
        $project = Project::orderBy('id', 'desc')->first();
        if (!empty($project)) {
            $project_id_string=str_replace('ALDAAR','',$project->project_no);
            $project_id='ALDAAR'.sprintf('%05d', intval($project_id_string)+1);
            
        }
        return $project_id;
    }

    public static function getLabourNo()
    {
        $ticket_id='LR00001';
        $ticket = ProjectWorkerRequest::orderBy('id', 'desc')->first();
        if (!empty($ticket)) {
            $ticket_id_string=str_replace('LR','',$ticket->ticket_id);
            $ticket_id='LR'.sprintf('%05d', intval($ticket_id_string)+1);
