...
<div class="modal-body">
    ...
    <div class="col-sm-6 col-md-6">
            <div class="form-group">
                ...
                {!! Form::select('type', $types, $project->type,array('class' => 'form-control','required'=>'required')) !!}
            </div>
        </div>
	<div class="col-sm-6 col-md-6">
            <div class="form-group">
                {{ Form::label('contract_type', __('Contract Type'),['class'=>'form-label']) }}<span class="text-danger">*</span>
                {!! Form::select('contract_type', $contract_type, $project->contract_type,array('class' => 'form-control','required'=>'required')) !!}
            </div>
        </div>
		</div>
    <div class="row">
            ...
            <div class="form-group">
                ...
                <select name="priority" id="priority" class="form-control main-element">
                    @foreach(\App\Models\Project::$project_priority as $k => $v)
                        <option value="{{$k}}" {{ ($project->priority == $k) ? 'selected' : ''}}>{{__($v)}}</option>
                    @endforeach
                </select>
            </div>
        </div>
