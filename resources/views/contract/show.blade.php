   ...
   <div class="col-xl-3">
       ...
       <div class="card report_card total_amount_card">
                <div class="card-header">
                   <h5 class="mb-0">{{ __('Contract Detail') }}</h5>
               </div>
            <div class="card-body">
                <ul class="basic-detail-box">
                    <li><span>{{ __('Subject') }}</span> {{ $contract->subject}}</li>
                    <li><span>{{ __('Project') }}</span> {{ !empty($contract->projects)?$contract->projects->project_name:'-' }}</li>
                    <li><span>{{ __('Value') }}</span> {{ \Auth::user()->priceFormat($contract->value) }}</li>
                    <li><span>{{__('Type')}}</span> {{ !empty($contract->types)?$contract->types->name:'-' }}</li>
                    <li><span>{{__('Status')}}</span> {{$contract->status }}</li>
                    <li><span>{{__('Start Date')}}</span> {{ Auth::user()->dateFormat($contract->start_date) }}</li>
                    <li><span>{{__('End Date')}}</span> {{ Auth::user()->dateFormat($contract->end_date) }}</li>
                </ul>
